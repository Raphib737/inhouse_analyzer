  ...
  render() {
    ...
    for (var i in d["role"]) {
      ...
      for (var c in d["role"][i]["champions"]) {
        ...
        for (var m in d["role"][i]["champions"][c]["match history"]) {
          ...
          c_h.push(
            <span>
              {o["result"].toUpperCase()} ({o["score"]}) vs {e["summoner"]}'s{" "}
              {e["champion"]} ({e["score"]})<br></br>
            </span>
          );
        }
        ...
        var champHistory = (
          <div className="tooltiptext">
            <div className="tooltipHeader">
              {c[0].toUpperCase() + c.slice(1).toLowerCase()} ({cS["won"]}-
              {cS["lost"]}) <br></br>Average Stats:(
              {cS["average kills"].toFixed(2)}/{cS["average deaths"].toFixed(2)}
              /{cS["average assists"].toFixed(2)})
            </div>
            {c_h}
          </div>
        );
        champPics.push(
          <div className="SciContainer">
            <img className="SchampImage" src={images[imgName]}></img>
            {champHistory}
          </div>
        );
      }
      roleElements.push(
        <div className="SroleContainer">
          <div className="SroleName">
            <img
              className="SroleImage"
              src={rolesImages[i.toLowerCase() + ".png"]}
            ></img>
          </div>
          <div className="Svision">
            ({d["role"][i]["won"]}-{d["role"][i]["lost"]})<br></br>Win Rate:
            {d["role"][i]["win rate"]}
            <br></br>
            Vision Score: {d["role"][i]["average vision score"]}
            <br></br>
            Game Time: {d["role"][i]["average game duration"]}
          </div>
          <div className="SchampPicks">
            Champion Pool<br></br>
            {champPics}
          </div>
        </div>
    ...
    return (
      <div className="SsummonerProfile">
        <div className="SsummonerName">
          Rank {d["rank"]}
          <br></br>
          {d["summoner"]} ({d["won"]}-{d["lost"]})<br></br>Win Rate:
          {d["win rate"]}
        </div>
        <div className="Sroles">
          <div className="SrolesHeader">Roles</div>
          {roleElements}
        </div>
        <div className="Spartners">
          <div className="SpartnersHeader"> Partners Win Rate</div>
          {partnersList}
        </div>
      </div>
    );
