...
class TeamGenerator extends Component {
  ...
  terrysSplit() {
    ...
    for (var i in redTeam) {
      ...
      teams["rRate"] += parseFloat(
        rankedTeams[redTeam[i]["index"]]["win rate"].slice(0, -1)
      );
  ...
  getTeams(players, avgWinrate, tolerance) {
    ...
    for (var i = 0; i < possibleTeams.length; i++) {
      possibleTeams[i].sort(function(combo1, combo2) {
  ...
  getCombinations(players, avgWinrate, tolerance) {
    ...
    for (var i = playersCopy.length - 1; i >= 0; i--) {
      ...
      var nextWinrateCombos = this.getCombinations(
        playersCopy,
        avgWinrate - currPlayer.winrate,
        tolerance
      );
  ...
  winRateSplit(e) {
    ...
    for (var i in order) {
      if (i < rankedTeam.length) {
        ...
        if (order[i] == "blueTeam") {
          teams["bRate"] += parseFloat(
            rankedTeam[index]["win rate"].slice(0, -1)
          );
          bp += 1;
        } else {
          teams["rRate"] += parseFloat(
  ...
  render() {
    ...
    if (typeof d == "object") {
      ...
      for (var i in this.state.players) {
        playersCon.push(
          <div className="GplayerCont">
            {parseInt(i) + 1}.) {this.state.players[i]}
            <button
      ...
      for (var i in bt) {
        ...
        btContainer.push(<div className="GpickedTeam"> {bt[i]}</div>);
      }
      ...
      for (var s in d["sorted_summoners"]) {
        summsDropdown.push(
          <div className="Gdropdown" onClick={this.addDropDownSummoner}>
            {d["sorted_summoners"][s]}
          </div>
        );
      }
    }
    ...
    return (
      <div className="dataContainer">
        <div className="GformInput">
          <div className="Gplayers">
            <div className="Gsumheader">Summoners</div>
            <br></br>
            {playersCon}
          </div>
          <div className="GdropdownContainer">
            <div className="dropdown">
              <span className="Gsdropdown"> Summoners Dropdown</span>
              <div className="dropdown-content">{summsDropdown}</div>
            </div>
          </div>

          <form className="Gform" onSubmit={this.handleSubmit}>
            <label>
              Add New Summoner<br></br>
              <input
                type="summonerName"
          ...
          <div className="Gbuttons">
            <button onClick={this.winRateSplit} value="ababbababa">
              WIN RATE SPLIT (ababbababa)
            </button>
            <br></br>
            <button onClick={this.winRateSplit} value="aabbbabbaa">
              WIN RATE SPLIT (aabbbabbaa)
            </button>
            <br></br>
            <button onClick={this.winRateSplit} value="abbbabaaab">
              WIN RATE SPLIT (abbbabaaab)
            </button>{" "}
            <br></br>
            <button onClick={this.randomSplit}>COMPLETELY RANDOM SPLIT</button>
            <br></br>
        ...
        <div className="Gteam Gblue">
          <div className="Gheader">
            Blue Team (Average Win Rate:&nbsp;{this.state.bRate}%)
          </div>
          {btContainer}
        </div>
        <div className="Gteam">
          <div className="Gheader">
            Red Team (Average Win Rate:&nbsp;{this.state.rRate}%)
          </div>
          {rtContainer}
        </div>
      </div>
